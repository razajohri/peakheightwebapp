...
function AuthPageContent() {
  ...
  const { user, signInWithEmail, signUpWithEmail, signInWithApple, signInWithGoogle, loading } =
    useAuth()
  ...
  const preferApple = useMemo(() => {
    if (typeof navigator === 'undefined') return false
    return /iPhone|iPad|iPod|Macintosh/.test(navigator.userAgent)
  }, [])

  const oauthReturnTo = redirectTo
  ...
  if (user) {
    if (fromOnboarding && justCompletedAuth) {
      return (
        <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-white">
          <div className="h-8 w-8 animate-spin rounded-full border-2 border-zinc-200 border-t-[#18181b]" />
          <p className="font-manrope text-sm text-[#a1a1aa]">Taking you to subscription…</p>
        </div>
      )
    }
    if (fromOnboarding) {
      return (
        <div className="flex min-h-screen flex-col items-center justify-center bg-white px-6">
          <p className="mb-2 text-center font-manrope text-lg font-semibold text-[#18181b]">
            You&apos;re already signed in
          </p>
          <p className="mb-6 text-center font-manrope text-sm text-[#a1a1aa]">
            Continue to choose your subscription plan.
          </p>
          <button
            type="button"
            onClick={() => router.push(redirectTo)}
            className="h-12 w-full max-w-md rounded-full bg-[#18181b] font-manrope font-medium text-white"
          >
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-white">
        <div className="h-8 w-8 animate-spin rounded-full border-2 border-zinc-200 border-t-[#18181b]" />
        <p className="font-manrope text-sm text-[#a1a1aa]">Redirecting…</p>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {error ? (
          <p className="mt-4 rounded-lg bg-red-50 py-2 px-3 text-center font-manrope text-[13px] text-red-600">
            {error}
          </p>
        ) : null}
        ...
        {!showEmail ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button
              type="submit"
              disabled={
                busy || !email || !password || (authMode === 'signup' && !confirmPassword)
              }
